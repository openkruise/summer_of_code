...
var (
	...
	envExample = templates.Examples(`
	  ...
	  env | grep RAILS_ | kubectl-kruise set env -e - deployment/registry`)
)
...
func NewCmdEnv(f cmdutil.Factory, streams genericclioptions.IOStreams) *cobra.Command {
	...
	cmd.Flags().BoolVar(&o.List, "list", o.List, "If true, display the environment and any changes in the standard format. this flag will removed when we have kubectl-kruise view env.")
